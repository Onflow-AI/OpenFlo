...
import os
import logging
from avenir_ux.prompts.utils import generate_action_journey_summary
from avenir_ux.agent.reporting import generate_comprehensive_action_summary
from avenir_ux.prompts.templates import (
...
async def should_terminate_intelligently(agent) -> bool:
    ...
    if len(agent.action_history) >= 25:
        recent_failures = sum(
            1
            for action in agent.action_history[-15:]
...
async def llm_should_terminate(agent) -> bool:
    ...
    try:
        ...
        action_summary = generate_comprehensive_action_summary(
            agent.taken_actions, agent.predictions
        )
        ...
        termination_prompt = build_termination_prompt(
            ...
            page_title,
            current_url,
            len(agent.taken_actions),
            action_summary,
        )

        # Check if screenshot exists before using it
        screenshot_for_termination = (
            agent.screenshot_path if os.path.exists(agent.screenshot_path) else None
        )
        response = await agent.engine.generate(
            prompt=[termination_prompt, termination_prompt, ""],
            image_path=screenshot_for_termination,
            turn_number=0,
        )
        ...
        if (
            ...
            and (isinstance(response, list) and len(response) > 0)
            or (isinstance(response, str) and response)
        ):
        ...
        if decision_text and decision_text.upper().startswith("TERMINATE:"):
            ...
            agent.logger.info(
                f"LLM recommends task termination based on semantic analysis. Justification: {justification}"
            )
            agent.complete_flag = True
            return True
...
async def should_terminate_on_failure(
    agent, failure_type: str, error_message: str
) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            page_indicators = {
                ...
                "has_error_message": any(
                    ...
                    for term in ["error", "forbidden", "denied"]
                ),
                "has_login_prompt": any(
                    term in page_content.lower()
                    for term in ["sign in", "login", "authenticate"]
                ),
            }
        ...
        try:
            ...
            if analysis_response and len(analysis_response) > 0:
                decision = (
                    analysis_response[0].upper()
                    if isinstance(analysis_response, list)
                    else str(analysis_response).upper()
                )

                if "TERMINATE" in decision:
                    agent.logger.info(
                        f"LLM recommends termination due to {failure_type} failure pattern or blocking condition"
                    )
                    return True
                elif "CONTINUE" in decision:
                    agent.logger.info(
                        f"LLM recommends continuing despite {failure_type} failures"
                    )
                    return False
        ...
        if (
            ...
            and len(agent.taken_actions) >= action_threshold
        ):
            agent.logger.info(
                f"High failure count ({recent_failures}) with sufficient actions ({len(agent.taken_actions)}) - recommending termination"
            )
            return True
...
async def verify_task_completion_before_terminate(agent) -> bool:
    ...
    try:
        ...
        checklist_context = ""
        if (
            hasattr(agent, "checklist_manager")
            and agent.checklist_manager.task_checklist
        ):
            checklist_status = agent.checklist_manager.get_checklist_status_summary()
            checklist_context = f"\n**CHECKLIST STATUS**\n{checklist_status}\n"

        completion_verification_prompt = build_completion_verification_prompt(
            agent.tasks[-1], action_journey, page_title, current_url, checklist_context
        )
        ...
        try:
            ...
            if (
                not image_paths
                and agent.screenshot_path
                and os.path.exists(agent.screenshot_path)
            ):
        ...
        if (
            ...
            if "NOT_COMPLETED" in decision or "NOT COMPLETED" in decision:
                agent.logger.info(
                    f"Task completion rejected by LLM. Reason: {decision_text}. Continuing execution."
                )
                return False
            elif "COMPLETED" in decision:
                agent.logger.info(
                    f"Task completion verified by LLM. Reason: {decision_text}. Terminating."
                )
                return True

        # If unclear response, use conservative approach
        agent.logger.warning(
            "Unclear LLM response for task completion verification, defaulting to not completed"
        )
        return False
...
async def evaluate_task_success(agent) -> str:
    ...
    is_explicit_completion = any(
        signal in last_action_type for signal in explicit_completion_signals
    ) or any(signal in action_generation for signal in explicit_completion_signals)
    ...
    has_completion_phrase = any(
        phrase in combined_text for phrase in completion_phrases
    )
    ...
    if len(agent.taken_actions) >= 2:
        for recent_action in agent.taken_actions[-3:]:  # Check last 3 actions
            ...
            if any(phrase in recent_generation for phrase in completion_phrases):
                ...
                agent.logger.info(
                    f"Completion signal detected in recent action: {recent_generation}"
                )
                break

    # If no completion signal detected, use LLM to generate natural description
    if (
        not is_explicit_completion
        and not has_completion_phrase
        and not recent_completion_detected
    ):
        agent.logger.info(
            "No explicit completion signal detected - generating LLM description"
        )
        ...
        try:
            ...
            if description_response and len(description_response) > 0:
                natural_description = (
                    description_response[0]
                    if isinstance(description_response, list)
                    else description_response or ""
                ).strip()
                if natural_description:
                    agent.logger.info(
                        f"Generated natural description: {natural_description}"
                    )
            ...
            return (
                "failure",
                f"The agent has taken {len(agent.taken_actions)} actions while working on the task. Currently on {page_title} page, but the task has not been completed yet.",
            )
            ...
            return (
                "failure",
                f"The agent is working on the task and has taken {len(agent.taken_actions)} actions so far. The task is still in progress.",
            )
    ...
    try:
        ...
        if (
            hasattr(agent, "session_control")
            and agent.session_control
            and isinstance(agent.session_control, dict)
            and agent.session_control.get("last_response")
        ):
            try:
        ...
        if hasattr(agent, "continuous_no_op"):
            additional_context.append(
                f"Continuous No-Op Count: {agent.continuous_no_op}"
            )

        # Add complete flag status
        additional_context.append(
            f"Complete Flag Status: {getattr(agent, 'complete_flag', False)}"
        )
        ...
        action_journey = generate_action_journey_summary(
            agent.taken_actions, agent.predictions
        )
        ...
        evaluation = await agent.engine.generate(
            prompt=[eval_prompt, eval_prompt, ""],
            image_path=screenshot_for_eval,
            turn_number=0,
        )
        ...
        if evaluation and len(evaluation) > 0:
            evaluation_text = (
                evaluation[0] if isinstance(evaluation, list) else evaluation or ""
            ).strip()
            ...
            agent.logger.info(
                f"Task evaluation after Agent completion signal: {status} - {description}"
            )
            ...
            return (
                "failure",
                "The agent signaled that it completed the task, but the evaluation process encountered an issue and couldn't provide a detailed assessment.",
            )
        ...
        return (
            "failure",
            f"The agent indicated it completed the task, but there was an issue during the evaluation process: {str(e)}",
        )

...
import traceback
import re
from avenir_ux.browser.dom import (
    dom_select_option,
    get_all_select_elements,
    decide_form_element,
    extract_typeable_elements,
    verify_last_typing,
    clear_active_field,
    looks_like_api_endpoint,
    choose_field_with_llm,
    dom_select_by_selector,
    extract_selectable_elements,
)
from avenir_ux.browser.recovery import find_click_target_by_text
...
async def perform_action(
    ...
    element_info = element_repr or (
        target_element.get("description") if target_element else None
    )
    coordinates = None
    if target_coordinates:
        if isinstance(target_coordinates, dict):
            ...
            isinstance(target_coordinates, (tuple, list))
            and len(target_coordinates) >= 2
        ):
            coordinates = (round(target_coordinates[0]), round(target_coordinates[1]))
        else:
            agent.logger.error(
                f"Invalid target_coordinates format: {type(target_coordinates)} - {target_coordinates}"
            )
            coordinates = None
    ...
    if action_name == "CLICK":
        ...
        if target_coordinates:
            try:
                if isinstance(target_coordinates, dict):
                    ...
                    click_failed = True
                    failure_reason = (
                        f"Invalid coordinate format: {type(target_coordinates)}"
                    )
                    raise ValueError(failure_reason)

                if not isinstance(llm_x, (int, float)) or not isinstance(
                    llm_y, (int, float)
                ):
                    raise ValueError(
                        f"Coordinates must be numeric: x={type(llm_x)}, y={type(llm_y)}"
                    )

                agent.logger.debug(
                    f"🔍 [CLICK] Raw coordinates: x={llm_x} (type: {type(llm_x)}), y={llm_y} (type: {type(llm_y)})"
                )
                ...
                try:
                    if (
                        getattr(agent, "_current_coordinates_type", "normalized")
                        == "normalized"
                    ):
                        scaled_x, scaled_y = agent.map_normalized_to_pixels(
                            llm_x, llm_y
                        )
                    else:
                        scaled_x, scaled_y = agent.normalize_coords(llm_x, llm_y, None)
                    agent.logger.info(
                        f"🎯 [CLICK] Coordinate Mapping: input({llm_x},{llm_y}) → Viewport({scaled_x},{scaled_y})"
                    )
                except Exception as norm_error:
                    raise ValueError(f"Coordinate normalization failed: {norm_error}")

                viewport_width = (
                    agent.config.get("browser", {})
                    .get("viewport", {})
                    .get("width", 1500)
                )
                viewport_height = (
                ...
                if not (
                    0 <= scaled_x < viewport_width and 0 <= scaled_y < viewport_height
                ):
                    agent.logger.warning(
                        f"⚠️ [CLICK] Normalized coordinates ({scaled_x}, {scaled_y}) outside viewport ({viewport_width}x{viewport_height}), clamping..."
                    )
                ...
                agent.logger.debug(
                    f"🖱️ [CLICK] Executing click at ({final_x}, {final_y}) with delay={delay}ms"
                )
                ...
                try:
                    ...
                    agent.logger.debug(
                        f"✅ [CLICK] Playwright click executed successfully"
                    )
                except Exception as click_error:
                    agent.logger.error(
                        f"❌ [CLICK] Playwright click failed: {type(click_error).__name__}: {click_error} | Trying mouse down/up fallback"
                    )
                    agent.logger.debug(
                        f"   Click params: x={final_x}, y={final_y}, delay={delay}"
                    )
                    try:
                        ...
                        agent.last_click_viewport_coords = (final_x, final_y)
                        agent.logger.debug(
                            f"✅ [CLICK] Fallback mouse down/up executed successfully"
                        )
                    except Exception as click_error2:
                        agent.logger.error(
                            f"❌ [CLICK] Fallback mouse down/up failed: {type(click_error2).__name__}: {click_error2}"
                        )
                        raise
                ...
                agent.logger.info(
                    f"✅ [CLICK] Successfully clicked at viewport coordinates ({final_x}, {final_y})"
                )
                ...
                click_failed = True
                failure_reason = (
                    f"Coordinate click failed - {type(e).__name__}: {str(e)}"
                )
        ...
        if click_failed and element_repr:
            agent.logger.info(
                f"⚠️ [CLICK] Coordinate click failed/missing, attempting fallback to text match for: {element_repr}"
            )
            found_elem, _, _ = await find_click_target_by_text(
                agent.page, element_repr, logger=agent.logger
            )
            if found_elem and found_elem.get("box"):
                ...
                agent.logger.info(
                    f"✅ [CLICK] Fallback found element by text at ({cx}, {cy})"
                )
        ...
        if target_coordinates and not target_element:
            try:
                ...
                if isinstance(target_coordinates, dict):
                    ...
                    agent.logger.error(
                        f"Invalid target_coordinates format for HOVER action: {type(target_coordinates)} - {target_coordinates}"
                    )
                    hover_failed = True
                    failure_reason = (
                        f"Invalid coordinate format: {type(target_coordinates)}"
            ...
            try:
                ...
                elif isinstance(target_element, dict) and target_element.get(
                    "center_point"
                ):
                    coords = target_element.get("center_point")
                    delay = random.randint(50, 150)
                    await agent.page.mouse.hover(
                        round(coords[0]), round(coords[1]), delay=delay
                    )
                    agent.logger.info(
                        f"Hovered over element at ({coords[0]}, {coords[1]}): {element_repr}"
                    )
                    return f"SUCCESS: Hovered over element at ({coords[0]}, {coords[1]}): {element_repr}"
                else:
                    raise ValueError(
                        "No selector or coordinates available for target element in HOVER"
                    )
            except Exception as e:
                hover_failed = True
        ...
        if not field_name:
            field_name = "unknown"
        agent.logger.info(
            f"🎯 [TYPE] Starting | Field: '{field_name}' | Value: '{value}' | Coords: {target_coordinates} | EnterAfter: {press_enter_after}"
        )
        coords = None
        if isinstance(target_coordinates, dict):
        ...
        if coords:
            try:
                ...
                delay = random.randint(50, 150)
                agent.logger.info(
                    f"🖱️ [TYPE] Clicking coordinates ({nx}, {ny}) to focus"
                )
                ...
                if await verify_last_typing(agent.page, value):
                    agent.logger.info(
                        f"✅ [TYPE L1/3] Success | Method: keyboard | Coords: ({nx}, {ny})"
                    )
                    if press_enter_after:
                        await agent.page.keyboard.press("Enter")
                        return (
                            f"SUCCESS: Typed '{value}' via keyboard and pressed Enter"
                        )
                    ...
                    agent.logger.warning(
                        f"⚠️ [TYPE L1/3] Failed | Method: keyboard | Error: {last_error} | Trying element selector"
                    )
            except Exception as e:
                attempts += 1
                last_error = str(e)
                agent.logger.warning(
                    f"⚠️ [TYPE L1/3] Failed | Method: keyboard | Error: {str(e)[:120]} | Trying element selector"
                )
        else:
            agent.logger.info(
                f"⚠️ [TYPE L1/3] Skipped | Reason: No coordinates provided | Trying element selector"
            )
            attempts += 1
        ...
        if selector:
            try:
                ...
                if await verify_last_typing(agent.page, value):
                    agent.logger.info(
                        f"✅ [TYPE ELEM] Success | Method: selector+keyboard | Element: {element_repr}"
                    )
                    if press_enter_after:
                        try:
                    ...
                    agent.logger.warning(
                        f"⚠️ [TYPE ELEM] Failed | Reason: {last_error} | Trying nearest input"
                    )
            except Exception as e:
                attempts += 1
                last_error = str(e)
                agent.logger.warning(
                    f"⚠️ [TYPE ELEM] Exception | Error: {str(e)[:160]} | Trying nearest input"
                )

        try:
            if coords:
                all_inputs = await extract_typeable_elements(
                    agent.page, logger=agent.logger
                )
                if all_inputs:
                    ...
                    if isinstance(sel_str, str) and sel_str.strip():
                        try:
                            ...
                            used_locator = False
                    if not used_locator:
                        await agent.page.mouse.click(
                            round(center["x"]), round(center["y"]), delay=delay
                        )
                    if clear_first:
                        await clear_active_field(agent.page)
                    await agent.page.keyboard.type(value)
                    agent.logger.info(
                        f"✅ [TYPE L2/3] Success | Method: nearest+keyboard | Field: {chosen_input.get('label') or chosen_input.get('placeholder') or 'Unnamed'}"
                    )
                    if press_enter_after:
                        await agent.page.keyboard.press("Enter")
                        return (
                    ...
                    agent.logger.warning(
                        f"⚠️ [TYPE L2/3] Failed | Reason: {last_error} | Trying GUI grounding"
                    )
                    attempts += 1
            else:
                all_inputs = await extract_typeable_elements(
                ...
                if all_inputs:
                    ...
                    def score_input(el):
                        ...
                        try:
                            w = float(
                                el.get("width", el.get("rect", {}).get("width", 0)) or 0
                            )
                            h = float(
                                el.get("height", el.get("rect", {}).get("height", 0))
                                or 0
                            )
                            if w * h > 0:
                                s += 1
                    ...
                    if not top_candidates:
                        ...
                        await agent.page.keyboard.type(value)
                        agent.logger.info(
                            f"✅ [TYPE L2b/3] Success | Method: semantic+keyboard | Field: {chosen_input.get('label') or chosen_input.get('placeholder') or 'Unnamed'}"
                        )
                        return f"SUCCESS: Typed '{value}' via keyboard"
                else:
                    attempts += 1
            ...
            last_error = str(e)
            agent.logger.warning(
                f"⚠️ [TYPE L2/3] Exception | Method: nearest+keyboard | Error: {str(e)[:120]}"
            )
        ...
        try:
            ...
            is_code_like = bool(
                re.match(
                    r"^(Key[A-Z]|Digit[0-9]|F[0-9]{1,2}|Arrow(Up|Down|Left|Right)|Page(Up|Down)|Enter|Tab|Escape|Backspace|Delete|Home|End)$",
                    content,
                )
            )
        ...
        try:
            vp = getattr(page, "viewport_size", None) or agent.config.get(
                "browser", {}
            ).get("viewport", {})
            vh = (
                vp.get("height", 720)
                if isinstance(vp, dict)
                else (vp.height if hasattr(vp, "height") else 720)
            )
            delta = -max(100, int(vh * 0.6))
            if isinstance(target_coordinates, dict) or (
                isinstance(target_coordinates, (tuple, list))
                and len(target_coordinates) >= 2
            ):
                if isinstance(target_coordinates, dict):
        ...
        if (
            ...
            and isinstance(agent.session_control, dict)
            and agent.session_control.get("context")
            and hasattr(agent.session_control["context"], "pages")
        ):
            ...
            if current_pages >= max_pages:
                agent.logger.warning(
                    f"Page limit reached ({current_pages}/{max_pages}). Cannot create new tab."
                )
                return f"SKIPPED: Page limit reached ({current_pages}/{max_pages})"

            new_page = await agent.session_control["context"].new_page()
            agent.logger.info(
                f"Opened a new tab (total pages: {len(agent.session_control['context'].pages)})"
            )
            return f"Opened a new tab (total pages: {len(agent.session_control['context'].pages)})"
        else:
            agent.logger.warning(
                "Cannot create new tab: session_control or context not properly initialized"
            )
        ...
        try:
            ...
            if ct and "text/html" not in ct:
                agent.logger.error(
                    f"Non-HTML content-type '{ct}' after GOTO {value}; reverting"
                )
                try:
                    await page.go_back()
                except Exception:
            ...
            if await agent._is_page_blocked_or_blank():
                agent.logger.error(
                    f"⛔ Page is blocked or blank after navigating to {value}"
                )
        ...
        try:
            ...
            if focused_element:
                agent.logger.info(
                    f"Found focused element for PRESS ENTER: {focused_element['tagName']} (type: {focused_element.get('type', 'N/A')})"
                )
            ...
            if search_element:
                agent.logger.info(
                    f"Found search element for PRESS ENTER: {search_element['tagName']} (selector: {search_element['selector']})"
                )
                await page.mouse.click(
                    search_element["center"]["x"], search_element["center"]["y"]
                )
                await page.keyboard.press("Enter")
                return f"SUCCESS: Clicked on {search_element['tagName']} element and pressed Enter"

            agent.logger.info(
                "No specific element found for PRESS ENTER, pressing Enter globally"
            )
            await page.keyboard.press("Enter")
            ...
            agent.logger.error(
                f"Enhanced PRESS ENTER failed, falling back to simple Enter: {e}"
            )
            await page.keyboard.press("Enter")
        ...
        if target_coordinates and not target_element:
            if value and "," in value:
                try:
                    ...
                    target_x, target_y = agent.map_normalized_to_pixels(
                        target_x, target_y
                    )
                    ...
                    if isinstance(target_coordinates, dict):
                        ...
                        try:
                            if (
                                getattr(
                                    agent, "_current_coordinates_type", "normalized"
                                )
                                == "normalized"
                            ):
                        ...
                        agent.logger.info(
                            f"Dragged from ({target_coordinates['x']}, {target_coordinates['y']}) to ({target_x}, {target_y})"
                        )
                        ...
                        agent.logger.info(
                            f"Dragged from ({target_coordinates[0]}, {target_coordinates[1]}) to ({target_x}, {target_y})"
                        )
                        return f"Dragged from ({target_coordinates[0]}, {target_coordinates[1]}) to ({target_x}, {target_y})"
                    else:
                        agent.logger.error(
                            f"Invalid target_coordinates format for DRAG action: {type(target_coordinates)} - {target_coordinates}"
                        )
                        drag_failed = True
                        failure_reason = (
                            f"Invalid coordinate format: {type(target_coordinates)}"
            ...
            try:
                ...
                if box:
                    ...
                    agent.logger.info(
                        f"Dragged element from ({source_x}, {source_y}) to ({target_x}, {target_y}): {element_repr}"
                    )
                    return f"Dragged element from ({source_x}, {source_y}) to ({target_x}, {target_y}): {element_repr}"
                else:
                    drag_failed = True
                    failure_reason = (
                        "Could not get element bounding box for drag operation"
                    )
                    agent.logger.error(failure_reason)
            except Exception as e:
                drag_failed = True
        ...
        if not field_name:
            field_name = "unknown"
        agent.logger.info(
            f"🎯 [SELECT] Starting | Field: '{field_name}' | Option: '{value}' | Coords: {target_coordinates}"
        )
        ...
        if coords:
            try:
                ...
                agent.logger.info(
                    f"🔧 [SELECT L1/2] Method: Pixel+DOM | Coords: ({nx}, {ny})"
                )
                success, message = await dom_select_option(agent.page, (nx, ny), value)
                if success:
                    agent.logger.info(
                        f"✅ [SELECT L1/2] Success | Method: Pixel+DOM | {message}"
                    )
                    return f"SUCCESS: {message} (method: pixel+DOM)"
                else:
                    agent.logger.warning(
                        f"⚠️ [SELECT L1/2] Failed | Method: Pixel+DOM | Reason: {message} | Trying Layer 2"
                    )
            except Exception as e:
                agent.logger.warning(
                    f"⚠️ [SELECT L1/2] Exception | Method: Pixel+DOM | Error: {str(e)[:100]} | Trying Layer 2"
                )
        else:
            agent.logger.info(
                f"⚠️ [SELECT L1/2] Skipped | Reason: No coordinates provided | Trying Layer 2"
            )

        try:
            agent.logger.info(
                f"🔧 [SELECT L2/2] Method: Semantic+LLM | Target field: '{field_name}' | Scanning page..."
            )
            ...
            if not all_selects:
                agent.logger.error(
                    f"❌ [SELECT L2/2] Failed | Reason: No dropdown fields found on page"
                )
                return f"FAILED: No dropdown fields available for selection"

            agent.logger.info(
                f"📋 [SELECT L2/2] Found {len(all_selects)} dropdowns | Asking LLM to find '{field_name}' dropdown..."
            )
            ...
            if chosen_select:
                ...
                agent.logger.info(
                    f"📋 [SELECT L2/2] LLM selected | Dropdown: {chosen_select.get('description')} | Selector: {selector}"
                )

                success, message = await dom_select_by_selector(
                    agent.page, selector, value
                )

                if success:
                    agent.logger.info(
                        f"✅ [SELECT L2/2] Success | Method: Semantic+LLM | {message}"
                    )
                    return f"SUCCESS: {message} (method: semantic+LLM)"
                else:
                    agent.logger.error(
                        f"❌ [SELECT L2/2] Failed | Method: Semantic+LLM | Reason: {message}"
                    )
                    return f"FAILED: {message}"
            else:
                agent.logger.error(
                    f"❌ [SELECT L2/2] Failed | Reason: LLM could not select appropriate dropdown"
                )
                return f"FAILED: No appropriate dropdown found for '{value}'"
        except Exception as e:
            agent.logger.error(
                f"❌ [SELECT] Failed | All methods exhausted | Error: {str(e)[:200]}"
            )
            agent.logger.debug(traceback.format_exc())
        ...
        if await agent._is_page_blocked_or_blank():
            agent.complete_flag = True
            agent.logger.info(
                "Page is blocked/error - allowing termination without task completion check"
            )
        ...
        if not say_text or say_text.lower() == "none":
            agent.logger.warning(
                "SAY action received with empty/None VALUE. Continuing execution."
            )
        ...
        agent.logger.info(
            "GUI_GROUNDING disabled: use main model actions with normalized coordinates"
        )
    ...
    if action_name in agent.no_element_op and target_element is None:
        ...
        if selector == "pixel_coordinates":
            ...
            if target_element and isinstance(target_element, dict):
                new_action = (
                    "[" + target_element.get("tag_with_role", "unknown") + "]" + " "
                )
                new_action += (
                    target_element.get("description", "unknown element")
                    + " -> "
                    + action_name
                )
            else:
...
async def execute(agent, prediction_dict):
    ...
    if prediction_dict is None:
        ...
        agent.logger.warning(
            "This may indicate max operations reached or prediction failure"
        )
        agent.complete_flag = True
        return
    ...
    if not isinstance(prediction_dict, dict):
        ...
        agent.logger.error(
            f"Expected dict, got {type(prediction_dict)}: {prediction_dict}"
        )
        agent.logger.error(
            "Marking task as complete due to invalid prediction structure"
        )
        agent.complete_flag = True
        return
    ...
    if pred_action is None:
        ...
        agent.logger.warning(
            f"pred_action is not a string: {type(pred_action)}, converting to string"
        )
    ...
    drag_source_desc = prediction_dict.get("drag_source_desc") or prediction_dict.get(
        "source_desc"
    )
    drag_target_desc = prediction_dict.get("drag_target_desc") or prediction_dict.get(
        "target_desc"
    )
    ...
    try:
        ...
        if pred_action == "TYPE" and pred_element is None and pred_coordinate is None:
            agent.logger.info(
                "TYPE action without element/coordinates - extracting available fields"
            )
            available_fields = await extract_typeable_elements(
                agent.page, logger=agent.logger
            )
            ...
            if available_fields:
                ...
                if chosen_field:
                    ...
                    pred_coordinate = (
                        chosen_field["center"]["x"],
                        chosen_field["center"]["y"],
                    )
                    pred_coordinate_type = "pixel"
                    agent.logger.info(
                        f"✅ Selected field for TYPE: {chosen_field.get('label') or chosen_field.get('placeholder') or 'Unnamed'}"
                    )
                    agent.logger.info(f"   Coordinates: {pred_coordinate}")
                else:
            ...
            pred_action == "SELECT" and pred_element is None and pred_coordinate is None
        ):
            agent.logger.info(
                "SELECT action without element/coordinates - extracting available dropdowns"
            )
            available_dropdowns = await extract_selectable_elements(
                agent.page, logger=agent.logger
            )

            if available_dropdowns:
                agent.logger.info(
                    f"Found {len(available_dropdowns)} selectable dropdowns"
                )
                ...
                if chosen_dropdown:
                    ...
                    pred_element = {
                        ...
                        "description": chosen_dropdown.get("label")
                        or chosen_dropdown.get("name")
                        or "Selected dropdown",
                        "center_point": (
                            chosen_dropdown["center"]["x"],
                            chosen_dropdown["center"]["y"],
                        ),
                        "tag_name": "select",
                    }
                    pred_coordinate = (
                    ...
                    pred_coordinate_type = "pixel"
                    agent.logger.info(
                        f"✅ Selected dropdown for SELECT: {chosen_dropdown.get('label') or 'Unnamed'}"
                    )
        ...
        # Only convert to NONE if BOTH element and coordinates are missing (after trying to find them above)
            ...
            pred_action == "DRAG"
            and pred_coordinate is None
            and (not pred_value or "," not in str(pred_value))
        ):
            agent.logger.warning(
                "DRAG requires source and target coordinates; converting to NONE"
            )
            pred_action = "NONE"
        elif (
            (pred_action not in agent.no_element_op)
            and pred_element == None
            and pred_coordinate == None
        ):
            if pred_action == "CLICK":
                target_text = (pred_value or "").strip() or (
                    pred_element_description or ""
                ).strip()
                if target_text:
                    found = await find_click_target_by_text(
                        agent.page, target_text, logger=agent.logger
                    )
                    if found:
                        pred_element, pred_coordinate, pred_coordinate_type = found
                    else:
                        agent.logger.warning(
                            f"Action {pred_action} requires element or coordinates, but both are None. Converting to NONE."
                        )
                        pred_action = "NONE"
                else:
                    agent.logger.warning(
        ...
        try:
            ...
            if (
                pred_action == "CLICK"
                and isinstance(pred_coordinate, (list, tuple))
                and len(pred_coordinate) >= 2
            ):
        ...
        new_action = await perform_action(
            ...
            pred_field,
            prediction_dict.get("action_description"),
            clear_first=prediction_dict.get("clear_first", True)
            if pred_action in ("TYPE", "KEYBOARD")
            else True,
            press_enter_after=prediction_dict.get("press_enter_after", False)
            if pred_action == "TYPE"
            else False,
        )
        ...
        if new_action is None:
            ...
            agent.logger.warning(
                f"Action: {pred_action}, Element: {pred_element}, Value: {pred_value}"
            )
            ...
            agent.logger.warning(
                f"=== PERFORM_ACTION RETURNED NON-STRING: {type(new_action)} ==="
            )
            agent.logger.warning(f"Value: {new_action}")
            new_action = (
                str(new_action)
                if new_action is not None
                else f"Action {pred_action} completed"
            )
        ...
        try:
            ...
            action_success = await agent._detect_page_state_change(
                state_before, state_after, pred_action
            )
        except Exception:
            action_success = True
        ...
        if not action_success:
            try:
                ...
                if agent.last_click_viewport_coords:
                    ...
                    isinstance(pred_coordinate, (list, tuple))
                    and len(pred_coordinate) >= 2
                ):
                ...
                try:
                    ...
                    labeled_screenshot = os.path.join(
                        agent.main_path,
                        "screenshots",
                        f"screen_{agent.time_step}_labeled.png",
                    )
                    agent.logger.info(
                        f"No-effect detected; labeled screenshot: {labeled_screenshot}"
                    )
                except Exception as ann_e:
                    agent.logger.warning(
                        f"Failed to annotate screenshot after no-effect: {ann_e}"
                    )
            except Exception as ss_e:
                agent.logger.warning(
                    f"Failed to capture/label screenshot after no-effect: {ss_e}"
                )
        ...
        enhanced_action = {
            ...
            "element_description": pred_element_description
            if pred_element_description
            else (pred_element.get("description", "") if pred_element else ""),
            "coordinates": pred_coordinate,
            "element_center": (
                pred_element.get("center_point") if pred_element else None
            ),
            "element_box": (pred_element.get("box") if pred_element else None),
            "http_response": agent.session_control.get("last_response", {})
        ...
        try:
            http_resp = (
                enhanced_action["http_response"]
                if isinstance(enhanced_action.get("http_response"), dict)
                else {}
            )
            agent.logger.info(
                f"  HTTP Response: url={http_resp.get('url')}, status={http_resp.get('status')}"
            )
        except Exception:
            pass
        ...
        try:
            ...
            new_action = (
                f"Failed to perform {pred_action} with value '{pred_value}': {e}"
            )
        ...
        try:
            ...
            agent.logger.error(
                f"Failed to take post-error screenshot: {screenshot_error}"
            )
        ...
        agent.taken_actions.append(enhanced_action)
        # Also add to action_history for failure analysis
        enhanced_action["failure_type"] = (
            "execution_error"  # Set failure type for analysis
        )
        agent.action_history.append(enhanced_action)
        agent.logger.info(f"Total actions in history: {len(agent.taken_actions)}")
        ...
        agent.logger.info(
            "Action failure detected - allowing LLM to re-evaluate (none option behavior)"
        )

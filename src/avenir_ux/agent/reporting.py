...
from avenir_ux.browser.helper import saveconfig
...
def generate_comprehensive_action_summary(
    taken_actions,
    predictions=[],
    ...
    if total_actions > max_actions and compress_old:
        ...
        summary_lines.append(
            f"=== ACTION HISTORY ({total_actions} total, showing recent {max_actions}) ==="
        )
    ...
    for i, action in enumerate(actions_to_process, start_index):
        if isinstance(action, dict):
            ...
            if (i - start_index + 1) <= len(predictions):
                ...
                thought = (
                    pred.get("action_generation", {}).get("thought", "")
                    if isinstance(pred.get("action_generation"), dict)
                    else ""
                )
            ...
            if thought:
                ...
                if "thought:" in action_generation.lower():
                    thought_part = (
                        action_generation.split("thought:")[-1]
                        .split("action:")[0]
                        .strip()
                    )
                    if thought_part:
                        summary_lines.append(
                            f"Reasoning: {_compress_text(thought_part, 200)}"
                        )
    ...
    successful_actions = sum(
        ...
        if isinstance(action, dict) and action.get("success", True)
    )
...
def emergency_save(task_id, taken_actions, error_info="Unknown error", logger=None):
    ...
    try:
        ...
        emergency_dir = os.path.join(
            tempfile.gettempdir(),
            f"avenir_ux_emergency_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
...
def save_results(
    main_path, task_id, final_json, taken_actions, config, logger, llm_io_records=None
):
                ...
                formatted.append(
                    {
                        ...
                        or ([r.get("image_path")] if r.get("image_path") else []),
                        "output": r.get("output"),
    ...
    try:
        with open(
            os.path.join(main_path, "all_predictions.json"), "w", encoding="utf-8"
        ) as f:
            try:
                filtered_records = [
                    r for r in (llm_io_records or []) if r.get("task_id") == task_id
                ]
            except Exception:
                filtered_records = []
        ...
        try:
            with open(
                ...
                json.dump(
                    {"error": f"Failed to save predictions: {str(e)}"}, f, indent=4
                )
        except Exception as fallback_e:
            if logger:
                logger.error(
                    f"Failed to save fallback all_predictions.json: {fallback_e}"
                )

    # Save result.json
    try:
        with open(
            os.path.join(main_path, "result.json"), "w", encoding="utf-8"
        ) as file:

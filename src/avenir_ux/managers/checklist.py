...
from avenir_ux.prompts.templates import (
    ...
    build_checklist_update_prompt,
)
from avenir_ux.utils.checklist import (
...
class ChecklistManager:
    def __init__(
        self, engine, logger=None, max_query_generations=5, checklist_engine=None
    ):
        """
        ...
        """
        self.engine = engine
        self.checklist_engine = (
            checklist_engine or engine
        )  # Use dedicated engine if provided
    ...
    def _extract_json(self, text):
        """Extract JSON object from text using regex and fallback methods."""
        if not text:
            return ""
    ...
    async def generate_task_checklist(self, task_description, strategic_reasoning=""):
        """
        ...
        if self.query_generation_count >= self.max_query_generations:
            self.logger.error(
                f"Maximum query generation attempts ({self.max_query_generations}) reached, checklist generation failed"
            )
        ...
        self.logger.info(
            f"Query generation attempt {self.query_generation_count}/{self.max_query_generations}"
        )
        ...
        try:
            checklist_prompt = build_checklist_prompt(
                task_description, strategic_reasoning
            )
            ...
            if not response:
                self.logger.error(
                    "Invalid response from engine.generate for checklist generation"
                )
                raise Exception(
                    "Failed to get valid response from LLM for checklist generation"
                )
            ...
            if isinstance(response, list):
                if not response:
                    self.logger.error(
                        "Empty response list from engine.generate for checklist generation"
                    )
                    raise Exception(
                        "Failed to get valid response from LLM for checklist generation"
                    )
                if response[0] is None:
                    self.logger.error(
                        "First element in response list is None for checklist generation"
                    )
                    raise Exception(
                        "Failed to get valid response from LLM for checklist generation - None element"
                    )
                ...
                self.logger.error(
                    "Unknown response format from engine.generate for checklist generation"
                )
                raise Exception(
                    "Failed to get valid response from LLM for checklist generation"
                )
            ...
            # Parse JSON response
            self.logger.info(f"Raw checklist response from LLM: {response_text}")
            ...
            try:
                ...
                self.logger.info(
                    f"Generated checklist with {len(self.task_checklist)} items"
                )
                for item in self.task_checklist:
                    self.logger.info(f"  - {item['description']}")
                ...
                # Fallback: create minimal checklist to ensure downstream logic works
                fallback_desc = (task_description or "Task").strip()
                self.task_checklist = [
                    {
                        "id": "execute",
                        "description": f"Execute task: {fallback_desc[:50]}...",
                        "status": "pending",
                    },
                    {
                        "id": "complete",
                ...
                self.checklist_generated = True
                self.logger.info(
                    "Checklist JSON parse failed; created fallback checklist"
                )
                return self.task_checklist

        except Exception as e:
            self.logger.error(f"Error generating checklist: {e}")
            # Fallback: create minimal checklist
            fallback_desc = (task_description or "Task").strip()
            self.task_checklist = [
                {
                    "id": "execute",
    ...
    def update_checklist_item(self, item_id, status, description=None):
        ...
        for item in self.task_checklist:
            if item["id"] == item_id:
                ...
                self.logger.info(
                    f"Checklist item '{item_id}' updated: {old_status} -> {normalized}"
                )
                if description:
    ...
    async def update_checklist_after_action(
        self,
        action_data,
        current_url,
        page_title,
        ...
        try:
            ...
            # Use lightweight checklist engine for faster updates
            response = await self.checklist_engine.generate(
            ...
            if not response:
                # Fallback update when engine returns empty
            ...
            if isinstance(response, list):
                ...
                # Unknown response format; apply fallback update
                self._apply_fallback_update(success, error)
                return
            ...
            try:
                ...
                for update in updates:
                    ...
                    if item_id and new_status and not applied:
                        ok = self.update_checklist_item(
                            item_id, new_status, description=None
                        )
                        if ok:
                            self.logger.info(
                                f"Checklist auto-update (atomic): {item_id} -> {new_status} ({reason})"
                            )
                            applied = True
                            break

                if not applied:
                    # Fallback update when no valid updates provided
                    self._apply_fallback_update(success, error)
                ...
                if self.task_checklist:
                    ...
                    self.logger.info(f"Checklist snapshot: {snapshot}")

            except json.JSONDecodeError:
                self.logger.warning(
                    f"Failed to parse checklist update JSON: {response_text}"
                )
                self._apply_fallback_update(success, error)
    ...
    def _apply_fallback_update(self, success, error):
        ...
        if target_idx is not None:
            ...
            if success and not error:
                new_status = (
                    "completed" if old_status == "in_progress" else "in_progress"
                )
            else:
                new_status = "failed"
            ...
            if ok:
                self.logger.info(
                    f"Checklist auto-update (fallback): {item.get('id')} -> {new_status}"
                )
        else:
    ...
    def add_checklist_item(self, description, item_id=None):
        ...
        new_item = {"id": item_id, "description": description, "status": "pending"}

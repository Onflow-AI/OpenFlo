...
import litellm
import asyncio
from avenir_ux.llm.engine import add_llm_io_record
# Note: build_summary_prompt and build_summary_update_prompt are imported inside functions
# to avoid circular import with templates.py
...
def analyze_repetitive_patterns(previous_actions):
    ...
    recent_actions = (
        previous_actions[-10:] if len(previous_actions) > 10 else previous_actions
    )
    ...
    if len(action_patterns) >= 3:
        for i in range(len(action_patterns) - 2):
            ...
            if (
                current[0] == next_1[0] == next_2[0]  # same action type
                and current[1] == next_1[1] == next_2[1]  # same element
                and current[2] == next_1[2] == next_2[2]  # same value
                and current[3]
                and next_1[3]
                and next_2[3]
            ):  # all failed
                warnings.append(
                    f"CRITICAL: You've attempted the identical action '{current[0]} {current[1]} {current[2]}' 3+ times and it keeps failing. STOP and try a completely different approach immediately!"
                )
                break
    ...
    if len(select_failures) >= 2:
        ...
        for (element, value), count in same_select_attempts.items():
            if count >= 2:
                warnings.append(
                    f"SELECT action failing repeatedly on '{element}' with value '{value}' ({count} times). The dropdown may not contain this option or the element may not be a proper select. Try CLICKING to open dropdown first, or look for alternative elements."
                )

    # 3. Check for excessive consecutive failures (lowered threshold)
    if consecutive_failures >= 2:
        warnings.append(
            f"You have {consecutive_failures} consecutive failed actions. CHANGE STRATEGY NOW - try scrolling, different elements, or a completely different approach."
        )

    # 4. Check for high failure rate in recent actions
    if len(recent_actions) >= 3 and failed_actions / len(recent_actions) > 0.5:
        warnings.append(
            f"High failure rate: {failed_actions}/{len(recent_actions)} recent actions failed. Your current approach is not working - RECONSIDER YOUR STRATEGY completely."
        )
    ...
    if len(action_types) >= 4:
        ...
        if len(set(last_4_actions)) == 1:  # All same action type
            ...
            if failed_count >= 3:
                warnings.append(
                    f"You've been repeating the same failing action type '{last_4_actions[0]}' for 4 consecutive steps with {failed_count} failures. TRY A COMPLETELY DIFFERENT ACTION TYPE (scroll, wait, or terminate)."
                )
    ...
    if len(recent_actions) >= 6:
        ...
        if recent_failed >= 5:
            warnings.append(
                "TERMINATION RECOMMENDED: You've failed 5+ times in the last 6 actions. This suggests the task may be impossible with current approach or the target element doesn't exist. Consider terminating or trying a fundamentally different strategy."
            )
    ...
    for i in range(len(action_patterns) - 1):
        ...
        if (
            current[0] == "SELECT"
            and current[3]  # SELECT failed
            and next_action[0] == "CLICK"
            and current[1] == next_action[1]
        ):  # followed by CLICK on same element
            select_click_pattern += 1

    if select_click_pattern >= 1:
        warnings.append(
            "Detected SELECT->CLICK fallback pattern repeating. The element may not be a proper dropdown. Try a different element, use TYPE instead, or choose a different action type. Avoid repeating the same failing pattern."
        )
    ...
    for pattern in action_patterns[-6:]:  # Check last 6 actions
        if pattern[3]:  # If action failed
            ...
            if element and element != "none":
                element_failure_count[element] = (
                    element_failure_count.get(element, 0) + 1
                )

    for element, count in element_failure_count.items():
        if count >= 2:
            warnings.append(
                f"Element '{element}' has failed {count} times in recent actions. Choose a different target or action type; avoid repeating the same failing interaction."
            )
    ...
    if len(recent_actions) >= 3 and failed_actions >= 2:
        ...
        warnings.append(
            "Multiple recent failures detected. Choose a fundamentally different approach: different action type, different element, or terminate if no viable path. Do not repeat the same action on the same element or coordinates."
        )
...
def generate_new_query_prompt(
    ...
    if repetition_detection and repetition_detection.get("has_repetition"):
        ...
        previous_action_text += (
            "You have been repeating the same actions without success.\n"
        )
        ...
        previous_action_text += (
            "If you cannot find a different approach, consider using TERMINATE.\n\n"
        )

    for i, action_item in enumerate(previous_actions):
        if isinstance(action_item, dict):
            logger.info(
                f"  Action {i + 1}: Step {action_item.get('step', 'N/A')} - {action_item.get('action_description', '')[:100]}..."
            )
            step = action_item.get("step", "N/A")
            action_type = action_item.get(
                "predicted_action", action_item.get("action", "UNKNOWN")
            )
            ...
            logger.info(
                f"  Action {i + 1}: {str(action_item)[:100]}... (legacy format)"
            )
...
def get_index_from_option_name(name):
    ...
    if len(name or "") == 1:
        ...
        print(
            f"ERROR: Invalid string length for get_index_from_option_name: '{name}' (length: {len(name or '')})"
        )
...
def generate_phase_summary(actions) -> str:
    ...
    action_summary = ", ".join(
        [f"{count} {atype}" for atype, count in action_counts.items()]
    )
...
def generate_action_journey_summary(taken_actions) -> str:
    ...
    if total_actions > 15:
        ...
        summary_lines.append(
            generate_phase_summary(taken_actions[middle_start:middle_end])
        )
    ...
    summary_lines.append(
        generate_detailed_action_list(taken_actions[-5:], start_idx=total_actions - 4)
    )
...
async def llm_summarize_actions(actions, engine, logger=None):
    from avenir_ux.prompts.templates import build_summary_prompt
    ...
    try:
        ...
        response = await engine.generate(
            prompt=["", prompt, ""], temperature=0.0, max_new_tokens=120, turn_number=0
        )
...
async def llm_update_history_summary(
    delta_actions, previous_summary, engine, logger=None
):
    from avenir_ux.prompts.templates import build_summary_update_prompt
    ...
    try:
        ...
        response = await engine.generate(
            prompt=["", prompt, ""], temperature=0.0, max_new_tokens=140, turn_number=0
        )
        if isinstance(response, list):
            return (response[0] or "").strip()
        if isinstance(response, str):
